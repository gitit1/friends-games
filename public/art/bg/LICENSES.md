...
| `shelf-rail.jpg` | warm oak shelf rail in shallow perspective (סדר יום / visual schedule strip) | drawn from scratch in-project (`scripts/gen-schedule-art.mjs`: a shallow receding top face with converging plank seams + horizontal grain, far-edge AO, a lit near edge and a thick front lip for depth) — the grounded surface the baked routine cards sit in; no third-party art | original (this repo) · CC0-equivalent |
| `potty-room.jpg` | calm home bathroom corner (גמילה מטיטול / potty training) | drawn from scratch in-project (`scripts/gen-potty-art.mjs`: a muted blue-greige wall with a soft window glow + understated frame + wainscot line, a bathroom floor tile in perspective with grout lines converging toward the back wall, baked AO at the wall/floor seam, per-tile brightness variance, corner vignette) — the grounded scene the baked toilet + sink sit in; no third-party art | original (this repo) · CC0-equivalent |
| `tracehe-desk.jpg` | warm wooden desk at a shallow angle, sibling of drawnum-desk (לצייר אות חיה / trace a Hebrew letter) | drawn from scratch in-project (`scripts/gen-tracehe-art.mjs`: warm wood plane, receding planks with baked perspective, dense grain fibers, soft top-right window light (the Hebrew reading corner), a small stack of muted alphabet blocks + a dusty-lilac pencil prop, vignette) — its own seed/composition so it reads as drawnum's sibling, not a re-skin; no third-party art | original (this repo) · CC0-equivalent |
