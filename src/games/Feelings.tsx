...
export default function Feelings({ onExit }: GameProps) {
  ...
  return (
    <GameShell title={t('game.feelings')} emoji="💚" onExit={onExit}>
      ...
      <div className={`fe-stage fe-ph-${phase}`} data-emo={emo}>
        <SceneBackdrop src="feelings-nook.jpg" position="center 30%" scrim="none" />
        <span className="fe-scrim" style={{ backgroundColor: zone.soft }} aria-hidden="true" />
        <p className="fe-bubble" key={phase + cue}>{bubble}</p>

        {phase === 'breathe' ? (
          <div className="fe-breathe-box">
            <span className={`fe-tool-fig fe-cue-${cue}`} data-tool={tool} aria-hidden="true">
              {TOOL_EMOJI[tool]}
            </span>
          </div>
        ) : (
