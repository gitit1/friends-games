...
// Prev/next now roam the WHOLE roster (1..friendCount()), wrapping — not just
// the level's band. The tier only picks the STARTING friend when the header
// difficulty changes; after that the child is free to walk every friend, all
// the way to the highest number available (friend 99 = 99 dots). Restored
// from the pre-7be64d7 browsing behaviour, layered on top of the tier system.
const stepRoster = (index: number, dir: number) => {
  const span = Math.max(1, friendCount())
  return ((index + dir) % span + span) % span
}
...
export default function ConnectDots({ onExit }: GameProps) {
  ...
  // The child now plays mostly on a TABLET, so let the worksheet grow generously
  // on wide screens (drawing area up to ~820px of usable width) instead of
  // staying phone-sized — while the 360px floor still fits with zero horizontal
  // overflow (win.w-based fraction + a modest scale cap keeps both true).
  // leave breathing room on the paper sheet around the friend (padding + tape)
  const boardCap = Math.min(win.w * 0.92, 820)
  const s = Math.min(boardCap / nat.w, (win.h * 0.46) / nat.h, 3.2)
      ...
      <div className="color-screen">
        <div className="dots-stepper">
          <Stepper
            ...
            onPrev={() => goTo(stepRoster(index, -1))}
            onNext={() => goTo(stepRoster(index, +1))}
          />
        </div>
