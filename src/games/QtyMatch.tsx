...
// אלוף = the game's historic maximum (groups up to 30 — docs/GAME-RULES.md:
// "אלוף הוא תמיד המאגר/הרמה הגדולים ביותר").
const QMAX_FOR_TIER = [3, 5, 8, 30]
...
const TOKEN_PX = 30 // per-member compact token size (countable, not tiny) — small groups

/** Token size shrinks as a group grows, so a 30-member אלוף group still fits its
 *  card (3-column grid, ~360px screen) with no overlap/overflow: members wrap
 *  inside the card and the card simply grows taller, but each token stays a
 *  clearly separate, tappable-looking crowd member down to the 8-group size that
 *  already shipped (≤8 keeps the original 30px). Counting-aloud is unaffected —
 *  it's driven by index, not pixel size. */
function tokenPxFor(count: number): number {
  if (count <= 8) return TOKEN_PX
  if (count >= 30) return 16
  // linear taper from 30px@8 down to 16px@30
  return Math.round(TOKEN_PX - ((count - 8) / (30 - 8)) * (TOKEN_PX - 16))
}
...
/** one compact crowd token (the cheap ~5-node friend), sized per the group's
 *  member count so large אלוף groups (up to 30) still fit their card. */
function Token({ index, counted, px }: { index: number; counted: boolean; px: number }) {
  return (
    <span className={`qty-mem ${counted ? 'is-counted' : ''}`}>
      <Friend index={index} scale={px / friendMaxDim(index)} showNumber={false} compact />
    </span>
  )
}
            ...
            {phase === 'seated' && (
              <div className="qty-guests">
                {correct.members.map((f, k) => {
                  // the seated-guests strip is a tighter box than the group cards
                  // (it lives inside the scene, above the tablecloth) — taper a
                  // little sooner so a 30-guest אלוף table still clears the sign.
                  const guestPx = Math.min(30, tokenPxFor(correct.count) - 4)
                  return (
                    <span key={k} className="qty-guest" style={{ ['--i' as string]: k }}>
                      <Friend index={f} scale={guestPx / friendMaxDim(f)} showNumber={false} compact />
                    </span>
                  )
                })}
              </div>
              ...
              <span className={`qty-cluster ${counting?.group === i ? 'is-counting' : ''}`}>
                {g.members.map((f, k) => (
                  <Token
                    key={k}
                    index={f}
                    counted={counting?.group === i && k < counting.lit}
                    px={tokenPxFor(g.count)}
                  />
                ))}
              </span>
            </button>
