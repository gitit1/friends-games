...
function makePuzzle(level: number): Puzzle {
  ...
  // A compact, uniform row of 6 cells (5 visible beads + the missing slot) at קל/
  // בינוני/קשה — the market-recommended "4–6 items", short enough to read on one
  // line in both directions. The unit repeats and the LAST bead is hidden (frozen
  // mechanic); 5 visible beads always show at least one full unit (max unit length
  // here is 4), so the answer is inferable. Difficulty grows through pattern
  // complexity and the number of distractors, not through a longer row.
  //
  // At אלוף (tier 3) the repo law requires the historic maximum: a variable-length
  // row from repeating the unit R∈{2,3} times (the original L×R idea), capped at 9
  // cells so it never outgrows the board. R≥2 guarantees a full unit is visible
  // before the missing slot, so the answer stays inferable.
  const R = level === 3 ? randInt(2, 3) : 0
  const N = level === 3 ? Math.min(9, L * R) : 6
...
export default function PatternGame({ onExit }: GameProps) {
  ...
  return (
    ...
    >
      ...
      <div className={`pat-stage center-body ${puz.seq.length > 6 ? 'pat-stage--long' : ''}`}>
        ...
        <div className="pat-track">
          <div
            className={`pat-seq ${puz.seq.length > 6 ? 'pat-seq--long' : ''}`}
            dir={dirFor(lang)}
            key={puz.seq.join('-')}
          >
            {puz.seq.map((hue, i) =>
