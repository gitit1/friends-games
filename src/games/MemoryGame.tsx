...
// FREE mode ("חופשי"): a second, child-facing way to size the board, living
// as an in-body pill + stepper (NOT in the header — the header stays the 4
// fixed tiers exactly as before). Card count is always even (cards come in
// pairs), from a comfortable MIN up to the whole (capped) cast, doubled.
const MIN_FREE_CARDS = 6
const FREE_DEFAULT_CARDS = 8
...
export default function MemoryGame({ onExit }: GameProps) {
  ...
  // FREE mode: an in-body opt-in that overrides the header tier with a child-
  // chosen card count. `free` is a plain toggle (not persisted — each visit
  // starts on the header tier, same as before); `freeCards` remembers the
  // stepper's position across the session so bumping ± doesn't reset it.
  const [free, setFree] = useState(false)
  const [freeCards, setFreeCards] = useState(FREE_DEFAULT_CARDS)
  // most cards = the whole (capped) cast, paired — the ceiling for the stepper
  const maxFreeCards = Math.min(friendCount(), 12) * 2
  const pairs = free ? Math.min(freeCards, maxFreeCards) / 2 : PAIRS_BY_TIER[level]
  ...
  // column count by density; the board fills up to 860px so it's big on a
  // tablet/desktop, and each friend fills its (responsive) card. Free mode can
  // go past the header's densest tier (up to 24 cards), so it gets its own
  // top step — still capped at 6 columns so cards never shrink to illegible.
  const cols = pairs <= 3 ? 3 : pairs <= 8 ? 4 : pairs <= 10 ? 5 : 6
  // The board itself may grow wider on tablets/desktop (up to 860px) so a
  // 24-card free board still gets comfortably big cards there, while phones
  // (vp.w) are unaffected — the 0.92 * viewport clamp already keeps 360px
  // screens overflow-free regardless of how many cards are on the table.
  const boardW = Math.min(860, vp.w * 0.92)
  ...
  // and switches OUT of free mode, since picking a tier is the child/parent
  // saying "use this preset now" (skip the first render — the initial board
  // is already dealt above).
  ...
  // turning FREE mode on deals a board at the current stepper count
  function enterFree() {
    unlockAudio()
    playTap()
    setFree(true)
    newDeck(Math.min(freeCards, maxFreeCards) / 2)
  }
  // ±2 cards at a time (always even — they come in pairs); redeals the board
  function changeFreeCards(delta: number) {
    const next = Math.max(MIN_FREE_CARDS, Math.min(maxFreeCards, freeCards + delta))
    if (next === freeCards) return
    unlockAudio()
    playTap()
    setFreeCards(next)
    newDeck(next / 2)
  ...
  return (
    <GameShell title={t('game.memory')} emoji="🧠" onExit={onExit} levels={{ gameId: 'memory', tiers: LEVEL_TIERS }}>
      ...
      <div className="memory-controls">
        <button className={`pill ${free ? 'pill-active' : ''}`} onClick={free ? undefined : enterFree} aria-pressed={free}>
          🎚️ {t('mem.open')}
        </button>
      ...
      {free && (
        // FREE mode's own stepper — pick the number of cards (even, in pairs).
        // dir="ltr" pins ➖ on the left / ➕ on the right in BOTH Hebrew and
        // English (a numeric stepper reads the same physical way either
        // direction, like a volume control), so this is an intentional
        // exception to the app's RTL mirroring rule, not a miss.
        // Buttons are 72×72 (min-height/min-width in app.css) — a real touch
        // target for the child, not the smaller 48px .btn-utility coin.
        <div className="memory-free" dir="ltr">
          <button
            className="memory-free-step"
            onClick={() => changeFreeCards(-2)}
            disabled={freeCards <= MIN_FREE_CARDS}
            aria-label={t('mem.fewer')}
          >
            ➖
          </button>
          <span className="memory-free-count">
            {freeCards} {t('mem.cards')}
          </span>
          <button
            className="memory-free-step"
            onClick={() => changeFreeCards(2)}
            disabled={freeCards >= maxFreeCards}
